...
class DesignerScene(QGraphicsScene):
    def __init__(self, parent=None):
        ...
        obj1 = UIObject()
        self.addItem(obj1)
        obj1.addComponent(UCRectangle)

        obj1 = UIObject()
        self.addItem(obj1)
        #obj1.addComponent(UCRectangle)
        obj1.addComponent(UCSprite,{"src":"/Users/choiyoung/Desktop/qwe.png"})
        '''
    ...
    def addObject(self,layerModel):
        cc = ComponentController.getInstance();

        obj = cc.Init(UIObject(layerModel));
        self.addItem(obj)

    def removeObject(self,layerModel):
        o = self.objectWithModel(layerModel)
...
class DesignerView(QGraphicsView):
    ...
    MOUSE_DRAG_OBJECT = 1
    MOUSE_DRAG_RUBBER = 2
    def __init__(self,parent,sceneModel):
        ...
        #connect
        LayerController.getInstance().itemAdded.connect(self.OnAddObject)
        LayerController.getInstance().itemDeleted.connect(self.OnDeletedObject)
    ...
    def OnAddComponent(self,layerModel,comp,number):
        o = self.scene.objectWithModel(layerModel)
        if o : o.addComponent(ComponentController.getInstance().ComponentInstance(comp),number)
        self.update()

    def OnDeleteComponent(self,layerModel,number):
        o = self.scene.objectWithModel(layerModel)
    ...
    def OnAddObject(self,LayerListModel,LayerModel):
        self.scene.addObject(LayerModel)

    def OnDeletedObject(self,layerListModel,layerModel):
        self.scene.removeObject(layerModel)
    ...
    def focusUpdate(self):
        ...
        for i in self.scene.selectedItems():
            layers.append(i.model)
        LayerController.getInstance().setFocusLayer(layers)
