...
@ffi.callback('int (*transfer_progress)(const git_transfer_progress *stats, void *payload)')
def _transfer_progress_cb(stats,payload):
	try:
		network_percent = (100*stats.received_objects) / stats.total_objects;
		#print "$$$$$$",network_percent
		#print my_progress
		if my_progress :
			my_progress(network_percent)
	except Exception, e:
		print e
	return 0
...
def clone_repository(
		url, path, progress=None, bare=False, ignore_cert_errors=False,
	...
	opts.bare = bare
	opts.remote_callbacks.transfer_progress = _transfer_progress_cb;
	if credentials:
